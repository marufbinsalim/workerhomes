...
import { create, modify, remove, update } from '@/lib/services/dwelling/gallery'
...
const GalleryStepWrapper = ({
  ...
  const handleDelete = async () => {
    ...
    if (selected.type === 'GALLERY') {
      const res = await remove(selected.id, tGallery('messages.delete-success'))
  ...
  const columns = [
    {
      ...
      Cell: item => <span className='text-brand'>{item?.amount || 0} zł</span>,
    },
    {
      Header: t('table.type'),
      Cell: item => (
